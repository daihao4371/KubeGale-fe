    ...
    <div class="dycloud-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <span>
          ...
          <el-date-picker v-model="searchInfo.startCreatedAt" type="datetime" placeholder="开始日期" :disabled-date="(time: Date) => searchInfo.endCreatedAt ? time.getTime() > searchInfo.endCreatedAt.getTime() : false"></el-date-picker>
          —
          <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="(time: Date) => searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        </el-form-item>
...
const searchRule = reactive<FormRules>({
  createdAt: [
...
// 搜索
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid: boolean) => {
    if (!valid) return
    page.value = 1
    pageSize.value = 10
    getTableData()
...
// 多选数据
const multipleSelection = ref<ClusterData[]>([])
// 多选
const handleSelectionChange = (val: ClusterData[]) => {
  multipleSelection.value = val
}

// 删除行
const deleteRow = (row: ClusterData) => {
...
const handleRbacManage = async(row: ClusterData) => {
  title.value = '集群授权'
  form.value = row
