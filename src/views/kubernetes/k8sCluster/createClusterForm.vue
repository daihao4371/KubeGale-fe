...
const props = defineProps({
  form: {
    ...
    type: Object as PropType<ClusterFormData>
  },
})
...
const GetFormData = () => {
  ...
  formData.value = props.form as ClusterFormData
...
// 关闭
const FormBlock = ref<FormInstance>()
...
const enterDialogs = async() => {
  if (!FormBlock.value) return
  FormBlock.value.validate(async (valid: boolean) => {
    if (valid) {
